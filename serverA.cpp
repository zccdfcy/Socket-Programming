...
int main(){
	...
	//read the file data1.txt
	ifstream data1;
	data1.open("data1.txt");
	if(!data1) {
		fprintf(stderr,"not find file data1.txt\n");
    	exit(1);
	}
	string countryList = "";
	...
	//get data from data1.txt line by line
	while(data1.getline(line_tmp, MAXLINESIZE)){
		//get country name
	...
	if ((rv = getaddrinfo(LOCAL_HOST, SERVERAUDPPORT, &hints, &servinfo)) != 0){
	...
	while(true){
		...
		if(msg == "Give me your country list!!!!"){
			//printf("serverA received the request from main server:%s\n",buf);
			//cout<<strlen(msg_countryList)<<endl;
